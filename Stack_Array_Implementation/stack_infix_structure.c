...
int getPrecedence(char ch)
{
	switch(ch)
	{
		...
		case '%': return 2;
		case '$': return 3;
		default: return -1;	//this case never 
	}
}
...
int main()
{
	...
	for(i=0; i<strlen(infix); i++) //read each symbol one by one from left to right
	{
		ch=infix[i];
		if(ch=='(')
			push(&opstack, ch); //push opener into the opstack
		else if(isalnum(ch)) //push the operand into the poststack
			push(&poststack, ch);
		else if(ch==')')
		{	//if scan char is closing paranthesis 
			while(peek(&opstack)!='(')	//see if opstack top contains opener
			{	push(&poststack, pop(&opstack)); }	//if not then pop and push into poststack
			pop(&opstack); //pop the opstack and discard the paranthesis
		}
		
		else	//if scan char is operator
		{
			while(!isEmpty(&opstack) && (getPrecedence(peek(&opstack))>=getPrecedence(ch)))	//opstack top 
	...
	while(!isEmpty(&opstack))	//until the opstack becomes empty
		push(&poststack, pop(&opstack));	//pop opstack and push into poststack
